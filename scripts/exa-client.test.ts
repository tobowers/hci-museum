...
test("the shared fallback budget caps parallel Exa requests at two", async () => {
  process.env.EXA_MAX_REQUESTS_PER_RUN = "2";
  ...
  expect(rejected).toHaveLength(28);
  expect(rejected.every((result) => result.reason instanceof ExaBudgetExceededError)).toBe(true);
});

test("fallback search always requests Exa's inexpensive fast type", async () => {
