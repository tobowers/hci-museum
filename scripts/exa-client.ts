...
export class ExaBudgetExceededError extends Error {
  constructor(limit: number) {
    super(
      `Exa fallback budget exhausted (${limit} per run). Continue with Octen, existing sources, Wikipedia, or direct page fetches; do not retry Exa.`,
    );
...
export async function searchExa(options: {
  ...
  console.error(`exa: fallback type=fast results=${data.results?.length ?? 0}${budgetLabel}${costLabel}`);
  return data;
}
